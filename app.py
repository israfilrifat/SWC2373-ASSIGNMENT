...
def index(): 
    if request.method == 'POST': 
        ...
        user_info = requests.get(f'{WEBEX_API_URL}/people/me', 
...
def rooms(access_token): 
    ...
    # Fetch rooms 
    rooms_data = requests.get(f'{WEBEX_API_URL}/rooms', 
headers=headers).json() 
    ...
    if request.method == 'POST': 
        ...
        send_message_response = requests.post(f'{WEBEX_API_URL}/messages', 
headers=headers, json=message_data) 
    ...
    return render_template('rooms.html', rooms=rooms_data['items'], 
access_token=access_token) 
